...
> *Entries are mean ± sample SD across three target seeds. For the learned-vector attack, AUC is first averaged across the three attacker seeds for each target.*
...
$$s_j = \frac{F_j}{F_j + N - g_j}$$

For a numeric factor and a displayed numeric product term, respectively:

$$s_j = \left\vert{}\operatorname{corr}(X_j,y)\right\vert{}, \qquad s_{jk} = \left\vert{}\operatorname{corr}(X_j X_k,y)\right\vert{}$$
...
Relative to QNN, the classical MLP improves test accuracy by +0.146 ± 0.093, with 95% confidence interval [0.062, 0.259]. Its generalization-gap and loss-AUC differences are not resolved. QCNN improves test accuracy by +0.190 ± 0.052, [0.148, 0.249], reduces the generalization gap by -0.041 ± 0.042, [-0.083, -0.007], and reduces loss-attack AUC by -0.019 ± 0.025, [-0.040, -0.001]. The HQNN intervals overlap zero.
...
| --- | --- | --- | --- | --- |
| Loss threshold | True-label probability and known candidate label | 0.596 ± 0.052 | 0.061 ± 0.024 | 0.135 ± 0.039 |
| Learned prediction-vector | Full prediction vector and derived statistics | 0.574 ± 0.065 | 0.099 ± 0.044 | 0.157 ± 0.061 |
| Online LiRA, fixed variance | True-label probability and calibrated reference QNNs | 0.609 ± 0.063 | 0.128 ± 0.056 | 0.192 ± 0.069 |
| Online LiRA, per-record variance | True-label probability and calibrated reference QNNs | 0.594 ± 0.057 | 0.086 ± 0.034 | 0.155 ± 0.063 |
| Offline LiRA, fixed variance | True-label probability and calibrated reference QNNs | 0.517 ± 0.029 | 0.066 ± 0.027 | 0.128 ± 0.035 |
| Label-only chord-boundary | Predicted labels only and held-out anchors | 0.582 ± 0.052 | 0.077 ± 0.027 | 0.139 ± 0.030 |
